...
def create_dashboard(df, port=8050):
    ...
    app.layout = html.Div([
        ...
        html.Div([
                ...
                html.Div([
                    dcc.Checklist(
                        ...
                        style={'max-height': '200px', 'overflow-y': 'auto', 'padding': '10px',
                               'backgroundColor': '#f5f9ff', 'borderRadius': '5px'}
                    ),
                ]),
                html.Div([
                    html.Button(
            ...
            ], style={'width': '23%', 'minWidth': '200px', 'padding': '10px', 'flex-grow': '1'}),
        ...
        html.Div([
            html.P(
                "👆 Click on any point in the graph to open the vehicle ad in a new tab")
    ...
    def update_submodel_options(selected_models):
        if not selected_models or len(selected_models) == 0:
            ...
            for sm in sorted(df['subModel'].unique()):
                ...
                models_for_submodel = df[df['subModel']
                                         == sm]['model'].unique()
    ...
    def update_graph(km_range, hand, models, submodel_btn_clicks, adtype, submodel_list):
        ...
        if submodel_list and len(submodel_list) > 0:
            ...
            filtered_df = filtered_df[filtered_df['subModel'].isin(
                submodel_list)]
        ...
        filtered_df['days_since_newest'] = (
            newest_date - filtered_df['productionDate']).dt.days

        # Calculate actual dates instead of days since newest
        today = pd.Timestamp.today().normalize()  # Get today's date (without time)
        filtered_df['display_date'] = today - \
            pd.to_timedelta(filtered_df['days_since_newest'], unit='D')
        ...
        fig = px.scatter(
            ...
            # Cap color scale for better differentiation
            range_color=[0, filtered_df['km_per_year'].quantile(0.95)],
            hover_data=['model', 'subModel', 'hand',
                        'km', 'city', 'productionDate', 'link'],
        ...
        if len(filtered_df) > 1:
            ...
            if len(x) > 1:  # Need at least 2 points for curve fitting
                try:
                    ...
                    curve_dates = [
                        newest_date - pd.Timedelta(days=int(days)) for days in curve_days]

                    # Add the exponential trendline with higher visibility
                    fig.add_trace(go.Scatter(
                        # Convert to actual dates
                        x=today - pd.to_timedelta(curve_days, unit='D'),
                        y=y_curve,
                        mode='lines',
                    ...
                    try:
                        ...
                        if len(x_valid) > 1:
                            ...
                            a = np.exp(z[1])
                            # Negative because our formula is exp(-bx)
                            b = -z[0]
                            ...
                            fig.add_trace(go.Scatter(
                                # Convert to actual dates
                                x=today - pd.to_timedelta(x_curve, unit='D'),
                                y=y_curve,
                                mode='lines',
                        ...
                        print(
                            f"Error with simplified exponential fit: {str(e2)}")
        ...
        summary = html.Div([
            ...
            html.Div([
                ...
                html.P(f"₪{filtered_df['price'].mean():,.0f}",
            ...
            html.Div([
                ...
                html.P(
                    f"₪{filtered_df['price'].min():,.0f} - ₪{filtered_df['price'].max():,.0f}", style=summary_style['value'])
            ], style=summary_style['card']),

            html.Div([
                html.P("Average km/year", style=summary_style['label']),
                html.P(
                    f"{filtered_df['km_per_year'].mean():,.0f}", style=summary_style['value'])
            ], style=summary_style['card']),

            html.Div([
                html.P("Average Vehicle Age", style=summary_style['label']),
                html.P(
                    f"{filtered_df['number_of_years'].mean():.1f} years", style=summary_style['value'])
            ], style=summary_style['card']),
...
def main():
    ...
    if not args.skip_scrape:
        scrape_data(args.output_dir, args.manufacturer,
                    args.model, args.max_pages)
