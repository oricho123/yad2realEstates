...
class RealEstateScraper:
    ...
    def fetch_listings(self,
        ...
        if min_square_meters:
            # Fixed: API expects minSquaremeter
            params['minSquaremeter'] = min_square_meters
        if max_square_meters:
            # Fixed: API expects maxSquaremeter
            params['maxSquaremeter'] = max_square_meters

        # Add priceOnly=1 parameter as shown in API example
        params['priceOnly'] = 1
        ...
        try:
            ...
            self.logger.info(
                f"Successfully fetched {len(data.get('data', {}).get('markers', []))} listings")
            return data
    ...
    def parse_listings(self, api_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for marker in markers:
            try:
                ...
                property_condition = additional_details.get(
                    'propertyCondition', {})
...
def main():
    ...
    if csv_path:
        ...
        try:
            ...
            print(
                f"📏 Size range: {df['square_meters'].min():.0f} - {df['square_meters'].max():.0f} sqm")
            print(
                f"�� Price/sqm range: ₪{df['price_per_sqm'].min():,.0f} - ₪{df['price_per_sqm'].max():,.0f}")

            print("\n🏠 Sample listings:")
            print(df[['price', 'rooms', 'square_meters',
                  'price_per_sqm', 'neighborhood']].head())
