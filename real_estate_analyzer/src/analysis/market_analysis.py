...
class MarketAnalyzer:
    ...
    def _analyze_neighborhoods(self) -> Dict[str, Any]:
        ...
        best_value_area = neighborhood_stats.loc[best_value_idx,
                                                 'neighborhood']
    ...
    def _analyze_value_opportunities(self) -> Dict[str, Any]:
        """Analyze value opportunities in the market using LOWESS."""
        try:
            # Use centralized LOWESS-based analysis
            analysis_df = TrendAnalyzer.calculate_complete_value_analysis(
                self.data)
            ...
            value_counts = analysis_df['value_category'].value_counts()
            excellent_deals = value_counts.get('Excellent Deal', 0)
            good_deals = value_counts.get('Good Deal', 0)
            undervalued = excellent_deals + good_deals
            overvalued = value_counts.get(
                'Above Market', 0) + value_counts.get('Overpriced', 0)

            # Calculate trend statistics
            x = self.data['square_meters'].values
            y = self.data['price'].values
            predicted_prices = analysis_df['predicted_price'].values
            trend_stats = TrendAnalyzer.get_trend_statistics(
                x, y, predicted_prices)
            ...
            return {
                ...
                'trend_slope': trend_stats.get('slope', 0),
                'trend_intercept': 0,  # LOWESS doesn't have a simple intercept
                'correlation': trend_stats.get('correlation', 0),
