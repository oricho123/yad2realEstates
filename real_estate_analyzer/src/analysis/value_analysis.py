...
class ValueAnalyzer:
    ...
    def calculate_value_scores(self) -> pd.DataFrame:
        """
        Calculate value scores for all properties based on market trend.
        ...
        try:
            ...
            # Fit polynomial (default: linear)
            self._trend_coefficients = np.polyfit(x, y, ValueAnalysisConstants.POLYNOMIAL_DEGREE)
            trend_line_y = np.poly1d(self._trend_coefficients)(x)
            
            # Calculate value scores (percentage above/below trend)
            value_scores = ((y - trend_line_y) / trend_line_y * 100)
            
            # Categorize properties
            value_categories = self._categorize_properties(value_scores)
            
            # Add to dataframe
            result_df = self.data.copy()
            result_df['value_score'] = value_scores
            result_df['value_category'] = value_categories
            result_df['trend_price'] = trend_line_y
    ...
    def get_best_deals(self, max_deals: int = 10) -> pd.DataFrame:
        ...
        good_deals['savings_amount'] = good_deals['trend_price'] - good_deals['price']
    ...
    def calculate_property_efficiency(self) -> pd.DataFrame:
        ...
        result_df['sqm_per_room'] = result_df['square_meters'] / result_df['rooms']
        result_df['sqm_per_room'] = result_df['sqm_per_room'].replace([np.inf, -np.inf], np.nan)
        
        # Calculate price per room
        result_df['price_per_room'] = result_df['price'] / result_df['rooms']
        result_df['price_per_room'] = result_df['price_per_room'].replace([np.inf, -np.inf], np.nan)
        ...
        if len(result_df) > 1:
            ...
            sqm_per_room_norm = (
                ...
                (result_df['sqm_per_room'].max() - result_df['sqm_per_room'].min())
            ).fillna(0.5)
            
            # Normalize price_per_sqm (lower is better)
            price_per_sqm_norm = 1 - (
                (result_df['price_per_sqm'] - result_df['price_per_sqm'].min()) /
                (result_df['price_per_sqm'].max() - result_df['price_per_sqm'].min())
            ).fillna(0.5)
            
            # Combined efficiency score
            result_df['efficiency_score'] = (sqm_per_room_norm + price_per_sqm_norm) / 2 * 100
        else:
            result_df['efficiency_score'] = 50  # Neutral score for single property
    ...
    def identify_outliers(self, method: str = 'iqr') -> pd.DataFrame:
        ...
        try:
            if method == 'iqr':
                ...
                result_df.loc[outliers, 'outlier_reason'] = 'Statistical outlier (Z-score > 2)'
