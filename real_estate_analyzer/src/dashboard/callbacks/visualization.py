...
class VisualizationCallbackManager:
    ...
    def _register_main_visualization_callback(self) -> None:
        ...
        def update_visualizations(price_range, sqm_range, neighborhood, exclude_neighborhoods, 
                                rooms, condition, ad_type, current_data):
            """
            ...
            """
            print(f"DEBUG: Visualization callback triggered with {len(current_data) if current_data else 0} properties")
            try:
                # Convert current data back to DataFrame
                if not current_data:
                    print("DEBUG: No current data available, returning empty visualizations")
                ...
                print(f"DEBUG: After filtering: {len(filtered_df)} properties remain")
                
                # If no data after filtering, return empty visualizations
                if filtered_df.empty:
                    print("DEBUG: No data after filtering, returning empty visualizations")
                    return self._get_empty_visualizations()
                ...
                return (
                    ...
                    self._create_summary_stats_display(summary_stats, len(filtered_df))
                )
        ...
        return html.Div([
            html.Div([
                ...
                html.Div([
                    ...
                    html.P(f"₪{price_stats.get('avg_price', 0):,.0f}", style=SummaryStyles.VALUE)
                ], style=SummaryStyles.CARD),
                
                # Price per SQM card
                html.Div([
                    html.H4("Avg Price/SQM", style=SummaryStyles.LABEL),
                    html.P(f"₪{price_stats.get('avg_price_per_sqm', 0):,.0f}", style=SummaryStyles.VALUE)
                ], style=SummaryStyles.CARD),
                
                # Average size card
                html.Div([
                    html.H4("Avg Size", style=SummaryStyles.LABEL),
                    html.P(f"{size_stats.get('avg_size', 0):.0f}m²", style=SummaryStyles.VALUE)
                ], style=SummaryStyles.CARD),
                
                # Average rooms card
                html.Div([
                    html.H4("Avg Rooms", style=SummaryStyles.LABEL),
                    html.P(f"{size_stats.get('avg_rooms', 0):.1f}", style=SummaryStyles.VALUE)
                ], style=SummaryStyles.CARD),
                
                # Data quality card
                html.Div([
                    html.H4("Data Quality", style=SummaryStyles.LABEL),
                    html.P(f"{data_quality.get('completeness_score', 0):.0f}%", style=SummaryStyles.VALUE)
                ], style=SummaryStyles.CARD),
