...
class PropertyDataFrame:
    ...
    def _ensure_data_types(self):
        ...
        numeric_columns = ['price', 'square_meters', 'price_per_sqm', 'lat', 'lng', 'rooms']
    ...
    def get_valid_properties(self) -> 'PropertyDataFrame':
        ...
        valid_data = self.data[
            ...
            (self.data['price_per_sqm'] <= PropertyValidation.MAX_REALISTIC_PRICE_PER_SQM)
        ].copy()
    ...
    def apply_filters(self, filters: PropertyFilters) -> 'PropertyDataFrame':
        ...
        if filters.min_square_meters is not None:
            filtered_data = filtered_data[filtered_data['square_meters'] >= filters.min_square_meters]
        if filters.max_square_meters is not None:
            filtered_data = filtered_data[filtered_data['square_meters'] <= filters.max_square_meters]
        
        # Room filters
        if filters.min_rooms is not None:
            filtered_data = filtered_data[filtered_data['rooms'] >= filters.min_rooms]
        if filters.max_rooms is not None:
            filtered_data = filtered_data[filtered_data['rooms'] <= filters.max_rooms]
        
        # Neighborhood filters
        if filters.neighborhoods and len(filters.neighborhoods) > 0:
            filtered_data = filtered_data[filtered_data['neighborhood'].isin(filters.neighborhoods)]
        
        if filters.exclude_neighborhoods and len(filters.exclude_neighborhoods) > 0:
            filtered_data = filtered_data[~filtered_data['neighborhood'].isin(filters.exclude_neighborhoods)]
        
        # Category filters
        if filters.property_types and len(filters.property_types) > 0:
            filtered_data = filtered_data[filtered_data['property_type'].isin(filters.property_types)]
        
        if filters.conditions and len(filters.conditions) > 0:
            filtered_data = filtered_data[filtered_data['condition_text'].isin(filters.conditions)]
        
        if filters.ad_types and len(filters.ad_types) > 0:
            filtered_data = filtered_data[filtered_data['ad_type'].isin(filters.ad_types)]
