...
class PropertyTableComponents:
    ...
    def create_best_deals_table(self, max_deals: int = 10) -> html.Div:
        ...
        for _, prop in best_deals.iterrows():
            ...
            location = f"{street}, {neighborhood}" if street and street.strip() else neighborhood
            ...
            if full_url and full_url.strip():
                title_element = html.A(
                    ...
                    style={'color': '#2c3e50', 'text-decoration': 'none', 'font-weight': '600',
                           'cursor': 'pointer', 'border-bottom': '1px dashed #667eea'}
                )
            else:
            ...
            row = html.Div([
                html.Div([
                    html.Div([
                        ...
                        html.P(f"Location: {location}", 
                              style={'margin': '5px 0 10px 0', 'color': '#6c757d', 'font-size': '14px'})
                    ], style={'flex': '1'}),
                ...
                html.Div([
                    html.Span(f"Price: {price_display}", 
                             style={'margin-right': '15px', 'font-weight': '600', 'color': '#2c3e50'}),
                    html.Span(f"{room_info}", 
                             style={'margin-right': '15px', 'color': '#495057'}),
                ...
                html.Div([
                    html.A(
                        [html.I(className="fas fa-external-link-alt", style={'margin-right': '5px'}), "View Listing"],
                        href=full_url,
                        target="_blank",
                        style={
    ...
    def create_market_insights_summary(self) -> html.Div:
        ...
        return html.Div([
            ...
            html.Div([
                ...
                html.Div([
                    html.H6("Affordability Analysis", style={'color': '#495057', 'margin-bottom': '10px'}),
                    html.P([
                        f"Most Affordable: ", 
                        html.Strong(f"{affordability_analysis['most_affordable']}", style={'color': '#28a745'}),
                        f" (Avg: ₪{affordability_analysis['most_affordable_price']:,.0f})"
                    ], style={'margin': '0 0 5px 0', 'font-size': '14px'}),
                    html.P([
                        f"Most Expensive: ", 
                        html.Strong(f"{affordability_analysis['most_expensive']}", style={'color': '#dc3545'}),
                        f" (Avg: ₪{affordability_analysis['most_expensive_price']:,.0f})"
                    ], style={'margin': '0 0 5px 0', 'font-size': '14px'}),
                    html.P([
                        f"Best Value: ", 
                        html.Strong(f"{affordability_analysis['best_value']}", style={'color': '#667eea'}),
                ...
                html.Div([
                    ...
                    html.P(f"{value_analysis['undervalued_count']} properties below market value",
                          style={'margin': '0 0 5px 0', 'font-size': '14px', 'color': '#28a745'}),
                    html.P(f"{value_analysis['overvalued_count']} properties above market value",
                          style={'margin': '0', 'font-size': '14px', 'color': '#dc3545'})
                ], style=insights_style['insight_card']),
    ...
    def _analyze_neighborhood_affordability(self) -> Dict[str, Any]:
        ...
        neighborhood_analysis['size_adjusted_price_per_sqm'] = (
            neighborhood_analysis['price_per_sqm'] * (neighborhood_analysis['square_meters'] / overall_avg_size)
        )
    ...
    def _analyze_value_opportunities(self) -> Dict[str, int]:
        ...
        try:
            ...
            undervalued_count = len(df_with_scores[df_with_scores['value_score'] < ValueAnalysisConstants.GOOD_DEAL_THRESHOLD])
            overvalued_count = len(df_with_scores[df_with_scores['value_score'] > ValueAnalysisConstants.FAIR_PRICE_THRESHOLD])
        except Exception:
            undervalued_count = 0
            overvalued_count = 0
    ...
    def create_summary_statistics_cards(self) -> html.Div:
        ...
        card_configs = [
            {'value': f"{stats['total_properties']}", 'label': 'Total Properties'},
            {'value': f"₪{stats['avg_price']:,.0f}", 'label': 'Avg Price'},
            {'value': f"₪{stats['avg_price_per_sqm']:,.0f}", 'label': 'Avg Price/SQM'},
