...
def test_number_formatting():
    ...
    for min_val, max_val in sqm_ranges:
        marks = NumberFormatter.create_number_marks(
            min_val, max_val, num_marks=5, suffix="m²")
...
def test_filter_integration():
    ...
    print(
        f"   Price range: ₪{sample_data['price'].min():,} - ₪{sample_data['price'].max():,}")
